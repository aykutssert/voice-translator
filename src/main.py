...
#     description="HTTP-based voice translation service",
#     version="3.0.0"
# )
...
#     """Application startup"""
#     logger.info("Starting Voice Translator HTTP API...")
#     await initialize_services()
#     initialize_firebase()
#     logger.info("Voice Translator HTTP API started successfully!")

# @app.on_event("shutdown")
# async def shutdown_event():
#     """Application shutdown"""
#     logger.info("Shutting down Voice Translator HTTP API...")
...
#     logger.info("Voice Translator HTTP API shutdown complete")
...
# async def process_audio_translation(audio_bytes: bytes, user_id: str) -> TranslationResponse:
#     """Process audio using OpenAI API and return translation"""
#     if not openai_client:
#         raise HTTPException(status_code=503, detail="Translation service unavailable")
    
#     # Check credits and estimate duration
#     duration_minutes = estimate_audio_duration(audio_bytes)
#     credits = await get_user_credits(user_id)
    
#     if not credits.get("is_admin", False) and credits.get("remaining_minutes", 0) < duration_minutes:
...
#         # Use OpenAI Whisper for translation
#         with open(temp_file, "rb") as audio_file:
#             translation = openai_client.audio.transcriptions.create(
#                 model="gpt-4o-transcribe",
#                 file=audio_file
#             )
#         turkish_text_TEST = translation.text

#         english_text_TEST = await translate_with_google(turkish_text_TEST, source='tr', target='en')

#         # print("whisper-1 translation result:")
#         # # Use OpenAI Whisper for translation
#         # with open(temp_file, "rb") as audio_file:
...
#         # # Reverse translate to Turkish using Google Translate
#         # turkish_text = await translate_with_google(english_text, source='en', target='tr')
#         # print(f"Turkish Text: {turkish_text}")
#         # Deduct credits
#         credits_used = 0.0
#         if not credits.get("is_admin", False):
#             await deduct_credits(user_id, duration_minutes)
#             credits_used = duration_minutes

#         # Get updated credits
#         updated_credits = await get_user_credits(user_id)

#         logger.info(f"Translation completed for user {user_id[:8]}...")
        
#         return TranslationResponse(
#             success=True,
#             turkish=turkish_text_TEST,
#             english=english_text_TEST,
...
# async def translate_with_google(text: str, source: str = 'en', target: str = 'tr') -> str:
...
#         # Mevcut kullanıcı için admin kontrolü
...
#             logger.info(f"Admin user {user_id} - no credit deduction")
#             return True
...
#         logger.info(f"Deducted {minutes_used:.2f} minutes from user. Remaining: {new_remaining:.2f}")
#         return True
...
# async def add_credits(user_id: str, minutes_to_add: int) -> bool:
...
#                 "total_purchased_minutes": minutes_to_add,
#                 "used_minutes": 0,
#                 "created_at": datetime.now(),
#                 "is_admin": False
#             })
...
#             "package_price": Config.PACKAGE_PRICE,
#             "timestamp": datetime.now()
#         })
        
#         logger.info(f"Added {minutes_to_add} minutes to user")
#         return True
...
#     return {
#         "message": "Voice Translator HTTP API",
#         "status": "running",
#         "version": "3.0.0",
...
#     """Translate audio from base64 encoded data"""
#     try:
#         # Decode base64 audio
#         try:
...
#         # Process translation
#         result = await process_audio_translation(audio_bytes, request.user_id)
...
#     user_id: str = Form(...)
# ):
#     """Translate audio from uploaded file"""
#     try:
#         # Validate file type
...
#         # Process translation
#         result = await process_audio_translation(audio_bytes, user_id)
...
#         if not all([user_id, product_id, transaction_id, minutes]):
...
#         # Add credits to user
#         success = await add_credits(user_id, minutes)
...
#             updated_credits = await get_user_credits(user_id)
#             logger.info(f"IAP: Added {minutes} minutes via {product_id}")
            
#             return {
#                 "success": True,
#                 "message": f"Successfully added {minutes} minutes",
...
# #     # Check rate limit (120 requests per minute for HTTP)
...
#     print(f"🌐 HTTP API Server starting on: http://{local_ip}:8000")
